...
| 機能 | 詳細 |
|---|---|
| 予測件数・エラー数 | 選択期間（直近1日 / 1週 / 1カ月 / 6カ月）で集計。実績値（actual_values）の連携件数・連携率も併記 |
...
| 自動更新 | 詳細: 30 秒、サマリー: 60 秒（`.env` の `AUTO_REFRESH_ENABLED=false` で無効化可） |
...
- **ドリフト判定はすべて画面の期間選択（直近1日 / 1週 / 1カ月 / 6カ月）と独立**。判定は常に「直近 `drift_window_size` 件」ベースのスナップショットであり、期間を切り替えても判定結果は変わらない（リクエスト集計・精度が期間連動なのと対照的）。
